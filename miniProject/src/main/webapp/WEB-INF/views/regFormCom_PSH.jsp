...
<script type="text/javascript">
    function validateForm() {
        ...
        var caddr1 = form["caddr1"].value.trim();
        if (caddr1 === "") {
            alert("회사 주소를 입력해주세요.");
            return false;
        }
        
        var cdocnum = form["cdocnum"].value.trim();
        if (cdocnum === "") {
            alert("사업자등록증명원발급번호를 입력해주세요.");
            return false;
        }
        
        var cmid = form["cmid"].value.trim();
        if (cmid === "") {
            alert("인사담당자 아이디를 입력해주세요.");
            return false;
        }
        
        var cmpw = form["cmpw"].value.trim();
        if (cmpw === "") {
            alert("인사담당자 비밀번호를 입력해주세요.");
            return false;
        }
        
        var cmemail = form["cmemail"].value.trim();
        var cmemail2 = form["cmemail2"].value.trim();
        if (cmemail === "" || cmemail2 === "") {
            alert("이메일 주소를 입력해주세요.");
            return false;
        }
        
        var cmtel2 = form["cmtel2"].value.trim();
        if (cmtel2 === "") {
            alert("인사담당자 전화번호를 입력해주세요.");
            return false;
        }
        
        var ctelverify = form["ctelverify"].value.trim();
        if (ctelverify === "") {
            alert("인증번호를 입력해주세요.");
            return false;
        }

        return true; // 모든 검증이 성공하면 폼을 제출합니다.
    }
</script>
</head>
<body>
...
<div>
    <form name="regForm" action="signUpCom" method="post" onsubmit="return validateForm()">
        <table>
            <tr>
                <td>사업자번호</td>
